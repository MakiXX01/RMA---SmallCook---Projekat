...
public class SQLiteManager extends SQLiteOpenHelper
{
    ...
    private static final String PREPARATION_TIME_FIELD = "preparationTime";
    ...
    public void addNoteToDatabase(Note note)
    {
        ...
        contentValues.put(PREPARATION_TIME_FIELD, note.getPreparationTime());
        contentValues.put(INGREDIENTS_FIELD, note.getIngredients());
        ...
        try (Cursor result = sqLiteDatabase.rawQuery("SELECT * FROM " + TABLE_NAME, null))
        {
            if(result.getCount() != 0)
            {
                while (result.moveToNext())
                {
                    ...
                    String preparationTime = result.getString(4);
                    String ingredients = result.getString(5);
                    String stringDeleted = result.getString(6);
                    Date deleted = getDateFromString(stringDeleted);
                    Note note = new Note(id,title,desc,preparationTime,ingredients,deleted);
